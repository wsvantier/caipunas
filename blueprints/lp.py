...
def classe(id):
    ...
    return render_template('turma.html', msg=erro, dados=dados, turma=busca.desc, id=busca.id)
...
def upload(id):
    try:
        ...
        db.session.rollback()
        return redirect(url_for('lp.classe', id=id, erro='Erro ao analisar o arquivo'))
